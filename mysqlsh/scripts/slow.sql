-- MySQL 5.5
SELECT start_time, user_host, SECOND(query_time) * 1000 AS 'query_time_microseconds', SECOND(lock_time) * 1000 AS 'lock_time_microseconds', rows_sent, rows_examined, db, last_insert_id, insert_id, server_id, CONVERT(sql_text USING utf8) as sql_text FROM mysql.slow_log;
